...
var Hour = /** @class */ (function () {
    function Hour(time) {
        ...
        // 使用当前的系统时间进行初始化
        if (time == undefined) {
            var _a = new Date().toLocaleString().split(" ")[1].split(":"), hours = _a[0], minutes = _a[1], seconds = _a[2];
            // 初始化秒位引用对象
...
var Date_ = /** @class */ (function () {
    function Date_(date) {
        // 初始化为当前日期
        if (date == undefined) {
            var _a = new Date().toString().split(" ")[0].split("/"), y = _a[0], m = _a[1], d = _a[2];
        ...
        // 以字符串初始化指定日期，例如 `2022/05/26`
            ...
            var _b = date.split("/"), yyyy = _b[0], mm = _b[1], dd = _b[2];
        ...
        // 以分别指定的指定年、月、日的形式初始化
            ...
            this.month = 0;
            this.day = 0;
            var f_param_type = typeof date[0];
            new ValueError("Type of first param is: " + f_param_type);
        }
    }
...
var DateTime = /** @class */ (function () {
    ...
    function DateTime(dtm) {
        var _a = dtm.split(" "), d = _a[0], t = _a[1];
